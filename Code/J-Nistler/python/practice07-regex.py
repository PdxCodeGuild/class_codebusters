...
def clean_phone_number(phone_number):
    regex = r'^\(?(\d{3})\)?[- ]?(\d{3})[- ]?(\d{4})'
    result_num = re.match(regex, phone_number)
    clean_num = ""
    if result_num:
        clean_num += result_num.group(1)
        clean_num += result_num.group(2)
        clean_num += result_num.group(3)
        return clean_num
...
def find_numbers(text):
    pattern = r'\-?\d+\.?\d+'
    str_results = re.findall(pattern, text)
    flt_list = []
    for str in str_results:
        flt_list.append(float(str))
    return flt_list
