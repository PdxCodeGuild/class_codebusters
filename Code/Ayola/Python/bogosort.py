...
# You can use list(password_string) or password_string.split('') to convert the string to a list, random.shuffle(password_list) to 
# shuffle it, and then ''.join(password_list) to turn it back into a string.