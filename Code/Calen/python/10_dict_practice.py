...
def create_contact(name: str, age: int) -> dict:
    '''returns a dictionary representing a contact given their name and age.'''
...
def find_key(d, value):
    return ''.join([key for key, val in d.items() if val == value]) if ''.join([key for key, val in d.items() if val == value]) != '' else None
...
def average_values(d):
    return {key: sum(value)/len(value) for key, value in d.items()  }
...
def merge_dictionaries(d1, d2):
    # return {key1: value1+value2 for (key1, value1), (key2, value2) in zip(d2.items(), d1.items())}
    return d2 | {key: d2.get(key, 0)+value for key, value in d1.items()}
...
def cart_total(prices, cart):
    return sum([item['quantity']*prices.get(item['item']) for item in cart ])
